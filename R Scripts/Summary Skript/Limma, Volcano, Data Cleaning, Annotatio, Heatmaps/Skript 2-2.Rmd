...
volcanizator <- function(data, logFC_thresh_upper, logFC_thresh_lower, max_pValue){
  ...
  # Plot
  ggplot(data, aes(x=log_FC, y=-log10(adj.P.Val))) +
...
```{r}
data.limma.p01andfc2.1 <- data.limma.p01andfc2[,1]
logFC <- (data.limma.p01andfc2.1/6-1)/100
data.limma.p01andfc2.2 <-data.limma.p01andfc2[,-1]
data.limma.p01andfc2.new <- cbind(logFC, data.limma.p01andfc2.2)
volcano <- volcanizator(data.limma.p01andfc2, 1, -1, 0.1)
```
...
ggplot(data=data.limma.p01andfc2, aes(x=logFC, y=-log10(adj.P.Val), col=diffexpressed, label=delabel)) +
        geom_point() + 
        theme_minimal() +
        geom_text_repel() +
...
pheatmap(correlation.mat_median, main = "Correlations between diffrent week differences")
...
# Heatmap for tissues
```{r}
data.liver = as.data.frame(embryo_df_tissues %>%filter(tissues == str_extract(tissues, "Liver")))
...
pheatmap(correlation.matrix, main = "Correlations between liver and kidney")
