...
```{r, fig.width=9, fig.height=6, fig.cap= "A. The number of transcripts associated with each tissue, including subtissues, is displayed.  B. The share of TRAs associated with each tissue. Subtissues are subsumed under their main tissue."}
...
```{r tissue-links,fig.width=7, fig.height=5, fig.cap= "Each tissue is displayed as a node with its size representing the number of transcripts associated with it. The edges show the shared TRAs between the linked tissues, with only links corresponding to more than 100 common TRAs visible."}
...
spleen_boxplot = ggplot(spleen_boxplot_data, aes(x = Cluster, y = Expression, fill = Week)) + geom_boxplot(position=position_dodge(0.8)) + theme_classic() + labs(title = "Expression of spleen-associated transcripts") + theme(aspect.ratio = 1)
spleen_boxplot
...
```{r ORA-code, include = FALSE, eval=FALSE}
...
```{r gene-expression-brain,fig.height=4,fig.width=12,fig.cap="Geneexpression of different genes for ion channel (left), neuronal development and function (middle) and cytokine (right) was plotted for week 4 to 9."}
...
Chemokinkines are important signaling molecules. They take up an important part in the signaling process during neuronal development (Tiveron 2008).## Hypothesis: Neural TRA expression patterns reflect morphological brain development
...
```{r pca-variance, fig.width = 3.5, fig.height=3.5}
...
```{r, eval=FALSE, fig.width = 12, fig.height = 7, fig.cap = "This is just a test..."}
