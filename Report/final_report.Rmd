...
The spleen first appears in the mesogastrium at week 6. Blood vessels appear in the organ in week 8 to 9 (James; 1983). The spleen plays an important role in immunesystem. As an example, b-lymphocytes are present in the spleen at week 12 (Hayward; 1983).
...
The skeletal muscles, e.g. some head muscles differentiate form mesoderm. First myotubes with central nuclei, where created by the fusion of myotubes between week 10 to 13. Typical muscle protein develop in the follwing weeks (Romero; 2013). 
...
Tissues specific antigens (TRAs) are important for the analysis of our dataset. TRAs are connected to the negative selection in thymus, which prevents an immune reaction against selfantigens. TRAs are ordered in cluster and controlled by autoimmune regulator (AIRE) in medullary epithelial cells (mTECs) and represent the diversity of antigens in our different tissues (Dinkelacker, 2019; Janeway Immunologie).We use the definition thate TRAs are genes, which are expressed more than 5 times the median in less than 5 different tissues (Dinkelacker, 2019). The role of TRAs has is target in this report.
...
```
Each transcript was awarded a rank depending on how much it contributes to the specific PCA. We took a closer look at the first five transcripts of PCA1 as this PCA alone already explains a big chunk of the variety of our data. These five transcripts encode for Low-Density Lipoprotein Receptor Class A Domain-Containing Protein 4, Calcium-activated potassium channel subunit beta-2 and SLIT And NTRK-Like Protein 3.
```{r}
PCA1_first_five <- diff_genes_ann_0.01[c('9703','11797','13044','616','15025'),0:19]
PCA1_first_five


chip1 <- PCA1_first_five[,c(1,4,7,10,13,16)]
chip2 <- PCA1_first_five[,c(2,5,8,11,14,17)]
chip3 <- PCA1_first_five[,c(3,6,9,12,15,18)]

plot(chip1)

PCA1_first_five %>% 
  ggplot(aes(x = 'chip1')) +
  geom_col(aes(y = c(4,5,6,7,8,9))) +
  labs(x = "Weeks", y = "Expression") +
  theme(aspect.ratio = 1)

plot(chip1, xlab = "weeks",ylab = "Expressionlevel")
```
...
# List the tissues
#tissues = c("Adipose", "Cells - Transformed fibroblasts", "Skin",  "Muscle - Skeletal", "Bladder", "Kidney - Cortex",  "Colon", "Esophagus", "Liver", "Minor Salivary Gland", "Pancreas", "Small Intestine - Terminal Ileum", "Spleen", "Stomach", "Breast - Mammary Tissue", "Cervix", "Fallopian Tube", "Ovary", "Uterus", "Vagina", "Prostate", "Testis", "Adrenal Gland", "Pituitary", "Thyroid",  "Artery", "Heart", "Lung", "Whole Blood", "Cells - EBV-transformed lymphocytes", "Brain - Spinal cord", "Brain", "Nerve - Tibial")
tissues = c("Adipose", "Cells - Transformed fibroblasts", "Skin", "Muscle - Skeletal", "Artery", "Heart")
...
for (tissue in tissues) {
  ...
  if (dim(enr_GO_tissue)[1] <8) {npath = dim(enr_GO_tissue)[1]} else {npath = 8}
  if(npath != 0) {
    data_plot = data.frame(Week = c(sapply(c(4:9), function(x) {rep(x, npath)})), Function =
                             as.character(c(rep(enr_GO_tissue$Description[1:npath], 6)), Expression=NA))
    for (i in 1:npath) {
  ...
  data_plot
    ...
    # Create the Plot
    assign(str_glue(tissue, "_plot"), ggplot(data_plot, aes(x = Week, y= Expression, group = Function))  + geom_line(aes(colour =Function), position = position_dodge(0.4)) + geom_point(aes(colour = Function), position = position_dodge(0.4)) + labs(title = tissue) +  theme_classic()   )  #ylim(6.5,8.5)
  }
  else {
    assign(str_glue(tissue, "_plot"), NA)
  }
   
}
ff = sapply(tissues, function(x){str_glue(x, "_plot")})
#grid.arrange(get(ff[1]), get(ff[2]), get(ff[3]), get(ff[4]), get(ff[5]), get(ff[6]))
```
...
get("Brain_table") 
ff = sapply(tissues, function(x){str_glue(x, "_table")})
for (i in 1:25) {
  get(ff[i])
}
...
assign(str_glue(tissue, "_plot"), ggplot(data_plot, aes(x = Week, y= Expression, group = Function))  + geom_line(aes(colour =Function), position = position_dodge(0)) + geom_point(aes(colour = Function, shape = Function), position = position_dodge(0)) + labs(title = tissue) +  theme_classic() + scale_color_npg()  + theme(legend.title = element_text(size = 9), legend.text = element_text(size = 7), aspect.ratio = 1, legend.position = "bottom", legend.spacing.y = unit(0.1, "cm"),legend.key.size = unit(0.4, "cm"), plot.margin = unit(c(0.2,0.5,0.3,0),"cm")) + guides(colour = guide_legend(nrow = length(selected), title.position = "top",byrow = TRUE), shape = guide_legend(nrow = length(selected), title.position = "top",byrow = TRUE)))
...
```{r, fig.width = 12, fig.height=16, fig.cap = ""}
...
plot_output = as_ggplot(plot) + draw_plot_label(label = c("A", "B", "C", "D", "E", "F", "G", "H"), size = 15, x = c(0.01, 0.51,0.01, 0.51,0.01, 0.51,0.01, 0.51), y = c(0.76,0.76,0.55,0.55,0.3,0.3,0.03,0.03)) 
