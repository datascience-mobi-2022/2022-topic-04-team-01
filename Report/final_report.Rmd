...
A single cell turns into a complex multicellular organism during embryogenesis. The morphological steps behind this process are mostly understood. The role of molecular biological pathways and molecules like chemokines in the embryonic development is quite unknown and has to be investigated. Tissue restricted antigens (TRA) may be a key in understanding the relation between molecular biological cause and morphological consequence.TRAs originate from negative selection in the thymus, where medullary epithelial cells (mTECs) express them on their surface to remove self-antigen reactive T-cells. Those TRAs represent a wide range of self-antigens, hence we can use them to determine the gene expression in our dataset (Dinkelacker 2019; Murphy & Weaver 2018).\
First of all, we want to compare our methods, e.g. limma analysis to the methods of the paper, e.g. **One-way analysis of variance** to find differentially expressed genes. We want to inspect if TRAs represent the timeline in the early stage of embryonic development between weeks 4 to 9 and if they can give us an insight on the molecular biological process in those weeks. Furthermore, we want to identify if the expression of specific gens can be related to milestones in neurogenesis. Last but not least, we want to illustrate the role of specific TRAs as biomarkers for organogenesis.
...
plot(data.ENST00000531293[,2],data.ENST00000531293[,1],type = "b",lwd=1,pch=19, main = "Ion channel gene", xlab = "weeks",ylab = "Expressionlevel",col = "red")
...
plot(data.ENST00000602349[,2],data.ENST00000602349[,1],type = "b",lwd=1,pch=19, main = "Neuronal development and function genes", xlab = "weeks",ylab = "Expressionlevel",col = "red", ylim = c(6.3,8.4))
...
plot(data.ENST00000337225[,2],data.ENST00000337225[,1],type = "b",lwd=1,pch=19, main = "Chemokine related genes", xlab = "weeks",ylab = "Expressionlevel",col = "red",ylim = c(7.6,9.7))
...
Fasold M. & Binder H., (2013), AffyRNADegradation: control and correction of RNA quality effects in GeneChip expression data, Bioinformatics, 29, 129-31. GTEX 2013, (2013), The Genotype-Tissue Expression (GTEx) project, GTEx Consortium, Nat Genet. 45, 580-5.
...
#PC 1&2
pc_12 <- ggbiplot(pca_genes) +
theme(aspect.ratio = 1)

#PC 2&3
pc_23 <- ggbiplot(pca_genes, choices=c(2,3)) +
theme(aspect.ratio = 1)

#PC 1&3
pc_13 <- ggbiplot(pca_genes, choices=c(1,3)) +
theme(aspect.ratio = 1)
